...
const ElementDownload = Tool.extend({
...
*/
    ...
    /**
     * Konvertiert Features, welche von dem Elementtools gesetzt wurden, nach KML, benötigt min. OpenLayers 3.12
    ...
    convertFeaturesToKML: function (features, context) {
        var format = new KML({extractStyles: true}),
            ...
            // arrays, in welche die Werte der Images gespeichert werden
            pointSource = [],
            pointRotation = [],
            pointSize = [],
            pointScale = [],
        ...
        _.each(features, function (feature) {
            var transCoord = this.transformCoords(feature.getGeometry(), this.getProjections("EPSG:25832", "EPSG:4326", "32")),
                ...
                color,
                style,
                // ++ speichert den text fuer die Abfrage
                size,
                text;
            ...
            //++ hat den gespeicherten namen der geometry 
            ...
            if (type === "Point") {
                ...
                if (!feature.getStyle().getText()) {
                    ...
                    //++ ueberprueft ob nur ein Punkt oder ein Element (spezieller Punkt) gesetzt werden soll.
                    if(text==="Element setzen"|| text==="Schild setzen"|| text==="geometry"  )
                    {
                        //++ setzt den Imagestyle
                        pointSource.push(style.getImage().getSrc());
                        pointRotation.push(style.getImage().getRotation());
                        size = style.getImage().getSize();
                        pointSize.push(size[0]+", "+size[1]);
                        pointScale.push(style.getImage().getScale());
                        pointText.push(text);
        ...
        // die Werte in KML-Tags einspeichern
        $(featuresWithPointStyle).find("Point").each(function (i, point) {
            ...
            if (!$(placemark).find("name")[0]) {
                ...
                // Sonderfall falls ein Image gesetzt werden soll. Also mehr als nur die Standardpunktwerte.
                if(text==="Element setzen"|| text==="Schild setzen"|| text==="geometry" )
                {
                pointStyle += "<icon>";
                pointStyle += "<src>"+ pointSource[i]+"</src>";
                pointStyle += "<rotation>"+ pointRotation[i]+"</rotation>";

                pointStyle += "<size>"+ pointSize[i]+"</size>";
                pointStyle += "<scale>"+ pointScale[i]+"</scale>";
                pointStyle += "</icon>";
                }
                pointStyle += "</pointstyle>";
